...
describe('app-capability-bridge Core', () => {
  ...
  test('retries ready ack after first send rejection and accepts a second session', async () => {
    ...
    await app.start(); app.receive(ready('first')); await Promise.resolve(); await Promise.resolve()
    expect(app.getSnapshot().state).toBe('connecting'); expect(logs.map(item => item.event)).toContain('ack.send.failure')
    app.receive(ready('first')); await Promise.resolve(); await Promise.resolve()
    expect(app.getSnapshot()).toMatchObject({ state: 'ready', sessionId: 'first', generation: 1 })
    app.receive(ready('second')); await Promise.resolve(); await Promise.resolve()
